...
class NotificationsViewModel constructor(
    ...
    private val _text = MutableLiveData<Event<String>>().apply {
        value = "This is notification Fragment".wrapWithEvent()
    }
            ...
            sharedWebService.getAllUserList().run {
                ...
                onSuccess {
                    ...
                    if (it.Status == VALID_STATUS_CODE) {
                        mDatum.value = it
                        _text.postValue("Notification Fragment SUCCES After Getting api Data".wrapWithEvent())

                    } else if (it.Status == 500) {
                        _text.postValue("Notification Fragment SERVER ERROR After Getting api Data".wrapWithEvent())

                        showSnackbarMessage(AppUtils.SERVER_NOT_RESPONDING_MESSAGE)

                    } else {
                        mDatum.value = it
                        _text.postValue("Notification Fragment UNKNOWN ERROR After Getting api Data".wrapWithEvent())
...
//                    it.message?.let { it1 -> showSnackbarMessage(it1) }
                    ...
                    _text.postValue("Notification Fragment Api Failed Data".wrapWithEvent())
